...
describe("Trending Component Tests", () => {
    it("Given the trending movies API call is successful, When the page loads, Then it should display the trending movies", () => {
      ...
      // Assert
      cy.get("[data-cy=trending-movie-container]").should("be.visible");
      cy.get("[data-cy=trending-movie-card]").should("have.length", 20); // Adjust the number as per your fixture data
    });
    ...
    it("Given the trending movies API call fails, When the page loads, Then it should display an error message", () => {
      ...
      // Assert
      cy.get("[data-cy=trending-movie-error-message]").should(
        "contain",
        "Server Error",
      );
      cy.get("[data-cy=trending-movie-card]").should("not.exist");
    });
  });
