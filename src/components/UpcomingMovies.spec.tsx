...
describe("UpcomingMovies", () => {
	beforeEach(() => {
		(movieApiClient.getMovieListUpcomingMovies as jest.Mock).mockReset();
	});
	...
	it("should fetch and display movies if the API call is successful", async () => {
		...
		(movieApiClient.getMovieListUpcomingMovies as jest.Mock).mockResolvedValueOnce({
			results: mockMovies,
		});
		...
		// Then
		expect(movieApiClient.getMovieListUpcomingMovies).toHaveBeenCalled();
	});
	...
	it("should display an error message if the API call fails", async () => {
		...
		// Then
		await waitFor(() =>
			expect(movieApiClient.getMovieListUpcomingMovies).toHaveBeenCalledTimes(1),
		);
